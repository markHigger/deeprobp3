...
def nn_forward_pass(params: Dict[str, torch.Tensor], X: torch.Tensor):
    ...
    hidden = X @ W1 + b1
    hidden = torch.max(hidden, torch.tensor(0))
...
def nn_forward_backward(
    ...
    prob = exp_scores / exp_scores.sum(dim=1, keepdim=True)
    # Probabilities of the correct class
    correct_class_probs = prob[torch.arange(X.shape[0]), y]  
    ...
    ###########################################################################
    # Replace "pass" statement with your code
    prob[torch.arange(X.shape[0]), y] -= 1  # Subtract 1 from the correct class probabilities
    prob /= X.shape[0]

    grads['W2'] = h1.T.mm(prob) + 2 * reg * W2  # Gradient of loss wrt W2 (with regularization)
    grads['b2'] = torch.sum(prob, dim=0)  # Gradient of loss wrt b2

    grad_hidden = prob.mm(W2.T)

    #only want to apply if relu was active
    grad_hidden[h1 <= 0] = 0

    grads['W1'] = X.T.mm(grad_hidden) + 2 * reg * W1  # Gradient of loss wrt W1 (with regularization)
    grads['b1'] = torch.sum(grad_hidden, dim=0)  # Gradient of loss wrt b1
    ###########################################################################
    #                             END OF YOUR CODE                            #
    ###########################################################################
...
def nn_predict(
    ...
    Inputs:
    - params: a dictionary of PyTorch Tensor that store the weights of a model.
      It should have followingåß keys with shape
          W1: First layer weights; has shape (D, H)
          b1: First layer biases; has shape (H,)
          W2: Second layer weights; has shape (H, C)
          b2: Second layer biases; has shape (C,)
...
def find_best_net(
    ...
    num_samples, input_size = data_dict['X_train'].shape
    output_size = 1
      ...
      for hs in hidden_sizes:
        for reg in regularization_strengths:
          for lrd in learning_rate_decays:
            net = TwoLayerNet(input_size, hs, 10, data_dict['X_train'].dtype)
            ...
            local_best_val = max(stats["val_acc_history"])
            if local_best_val > best_val_acc:
              best_val_acc = local_best_val
              best_stat = stats
              best_net = net
