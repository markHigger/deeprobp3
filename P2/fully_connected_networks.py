...
class Linear(object):
    ...
    def forward(x, w, b):
        ...
        x_inputs = x.flatten(1)
        # print((torch.mm(x_inputs,w)).shape)
        out = torch.mm(x_inputs,w) + b
        ######################################################################
        #                        END OF YOUR CODE                            #
        ######################################################################
        cache = (x, w, b)
    ...
    def backward(dout, cache):
        ...
        # dx = torch.mm(w, dout.T).T
        dx = dout @ w.T 
        dx = torch.reshape(dx, x.shape)
        # print(dx.shape)


        # dw = torch.mm(x_inputs.T, dout)
        dw = x_inputs.T @ dout
        # print(dw.shape)
...
class TwoLayerNet(object):
    ...
    def loss(self, X, y=None):
        ...
        # Replace "pass" statement with your code
        # print(X)
        # scores, cache1 = act_lin.forward(X, self.params['W1'],  self.params['b1'])
        # fc_cache, relu_cache = cache
        h1, cache1 = self.layer1.forward(X, self.params['W1'], self.params['b1'])
        
        # Forward pass through second layer (Linear)
        scores, cache2 = self.layer2.forward(h1, self.params['W2'], self.params['b2'])

        # print(h1)
        # scores = act_relu.forward(h1, self.params['W2'], self.params['b2'])
        ...
        _, grads['W2'], grads['b2'] = self.layer2.backward(dx, cache2)

        dout1 = dx.mm(self.params['W2'].T)  # Gradient w.r.t. out1

        _, grads['W1'], grads['b1'] = self.layer1.backward(dout1, cache1)


        #grads['W1'] = X.T.mm(grad_hidden) + 2 * reg * W1
...
class FullyConnectedNet(object):
    ...
    def __init__(self, hidden_dims, input_dim=3*32*32, num_classes=10,
        ...
        # Replace "pass" statement with your code
        self.layers = []
        input_dim_new = input_dim
        for layer_num in range(self.num_layers - 1):
            self.layers.append(Linear_ReLU())
            output_dim = hidden_dims[layer_num]
            self.params['W'+ str(layer_num)] = torch.randn((input_dim_new, output_dim), dtype = dtype, device=device) * weight_scale
            self.params['b'+ str(layer_num)] = torch.zeros((output_dim), dtype = dtype, device=device)
            input_dim_new = output_dim
        
            # print(self.params['W'+ str(layer_num)].shape)

        output_dim = num_classes
        self.params['W'+ str(layer_num+1)] = torch.randn((input_dim_new, output_dim), dtype = dtype, device=device) * weight_scale
        self.params['b'+ str(layer_num+1)] = torch.zeros((output_dim), dtype = dtype, device=device)
    ...
    def loss(self, X, y=None):
        ...
        layer_input = X
        for layer_num in range(self.num_layers - 1):
            layer_output, layer_cache = self.layers[layer_num].forward(layer_input, 
                                                 self.params['W'+ str(layer_num)], 
                                                 self.params['b'+ str(layer_num)])
            layer_caches.append(layer_cache)
            if self.use_dropout:
                layer_output, dropout_cache = Dropout.forward(layer_output, self.dropout_param)
                dropout_caches.append(dropout_cache)  # Store dropout cache
        ...
        # Forward pass through second layer (Linear)
        final_layer = Linear()
        scores, cache_final = final_layer.forward(layer_input, 
                                                  self.params['W'+ str(layer_num+1)], 
                                                  self.params['b'+ str(layer_num+1)])
        layer_caches.append(cache_final)
        #################################################################
        #                      END OF YOUR CODE                         #
        #################################################################
        ...
        for layer_num in reversed(range(self.num_layers)):
            if layer_num == self.num_layers - 1:
              dx, grads['W'+ str(layer_num)], grads['b'+ str(layer_num)] = \
                self.layers[layer_num].backward(dout, layer_caches[layer_num])
            else:
                if self.use_dropout:
            ...
            # print(layer_num)
            # dx, grads['W'+ str(layer_num)], grads['b'+ str(layer_num)] = \
            #         self.layers[layer_num].backward(dout, layer_caches[layer_num-1])
            
            # dout = dx
            # _, grads['W'+ str(layer_num)], grads['b'+ str(layer_num)] = \
            #         self.layers[layer_num].backward(dout, layer_caches[layer_num])
            
            # dout = dout.mm(self.params['W'+ str(layer_num)].T)
        ...
        # dout1 = dx.mm(self.params['W2'].T)  # regularize
...
def create_solver_instance(data_dict, dtype, device, **kwargs):
    ...
    solver = None
    # Replace "pass" statement with your code
    solver = Solver(model= model, data=data_dict, **kwargs)
    ##############################################################
    #                    END OF YOUR CODE                        #
    ##############################################################
    return solver
...
def rmsprop(w, dw, config=None):
    ...
    grad = decay*grad + (1 - decay)*(dw**2)
    next_w = w - lr * dw / torch.sqrt(grad + eps)
    config['cache'] = grad
...
def adam(w, dw, config=None):
    ...
    next_w = w - (m_hat * lr / (torch.sqrt(v_hat) + eps))
